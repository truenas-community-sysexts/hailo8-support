...
- **Frigate-pin gate on Hailo bumps.** Caps the candidate version at the HailoRT version Frigate pins on the `dev` branch: `HAILORT_VERSION` in `frigate/detectors/plugins/hailo.py` (Frigate removed the earlier source, `docker/main/install_hailort.sh`, in September 2026), which must agree with `hailo_version` in `docker/hailo8l/user_installation.sh`.
