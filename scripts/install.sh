...
#
# Usage: curl -fsSL https://raw.githubusercontent.com/truenas-community-sysexts/hailo8-support/main/get.sh | sudo bash
#        (get.sh picks the approved release and runs THAT release's install.sh
#        on its own verified hailo.raw)
...
if [ -n "$LOCAL_RAW" ]; then
    ...
    # The newest build a hardware test approved for this box's TrueNAS train,
    # built for its running kernel (see the approved-release block above).
    # The TrueNAS version decides the channel (stable vs preview) and the
    # train; it is never inferred from the kernel number.
    RELEASE_TAG=$(approved_release_tag install) || exit 1
