...
# on a kernel with no approved build they use the newest approved release
# built for this train (never another train's, whose scripts target that
# train's install layout; with none, they stop and name the waiting hardware
# tests, and --release=TAG is the way to pick one). --repo=OWNER/NAME (or
# HAILO_REPO) points all of it at a fork.
